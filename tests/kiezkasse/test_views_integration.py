...
def test_detail_view(client, phase_factory, proposal_factory, area_settings_factory):
    ...
    with freeze_phase(phase):
        response = client.get(url)
        assert_template_response(response, "meinberlin_kiezkasse/proposal_detail.html")
...
def test_create_view(
    ...
    url = reverse(
        "meinberlin_kiezkasse:proposal-create", kwargs={"module_slug": module.slug}
    )
    with freeze_phase(phase):
...
def test_moderate_view(
    ...
    area_settings_factory(module=module)
    url = reverse(
        "meinberlin_kiezkasse:proposal-moderate",
