...
def test_chapter_detail_view(client, phase_factory, chapter_factory):
    phase, module, project, item = setup_phase(phase_factory, None, phases.CommentPhase)
...
def test_paragraph_detail_view(
    client, phase_factory, chapter_factory, paragraph_factory
):
    phase, module, project, item = setup_phase(
        phase_factory, chapter_factory, phases.CommentPhase
    )
    ...
    with freeze_phase(phase):
        response = client.get(url)
        assert_template_response(response, "meinberlin_documents/paragraph_detail.html")
