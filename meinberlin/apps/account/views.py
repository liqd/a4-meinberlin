...
from meinberlin.apps.users.forms import GuestConvertForm
...
class GuestConvertView(
    ...
    template_name = "meinberlin_account/guest_convert.html"
    form_class = GuestConvertForm

    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        kwargs["user"] = self.request.user
        # ConvertFormView injects a model instance; our form is a plain signup
        # form that operates on the guest user passed above.
...
class NotificationsView(RegularUserRequiredMixin, generic.TemplateView):
    model = Action
...
class FollowedProjectsListView(
    LoginRequiredMixin, RegularUserRequiredMixin, generic.ListView
):
