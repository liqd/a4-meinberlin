...
def test_get_footer_with_download(clear_cache_footer, requests_mock):
...
def test_task_footer_if_no_footer_url_in_settings(clear_cache_footer):
...
def test_task_footer_if_no_footer_filepath_in_settings(clear_cache_footer):
    downloaded = periodic_footer_update()
    assert downloaded is False
    footer = cache.get("footer")
    assert footer is None
...
def test_task_footer_if_no_footer_settings(clear_cache_footer):
    downloaded = periodic_footer_update()
    assert downloaded is False
    footer = cache.get("footer")
    assert footer is None
...
@override_settings(BERLIN_FOOTER_URL="", BERLIN_FOOTER_FILENAME="")
def test_command_get_footer_if_no_footer_settings(clear_cache_footer):
...
def test_command_get_footer(clear_cache_footer, requests_mock):
    mock_footer = bytes("<p> berlin footer </p>", "utf-8")
    requests_mock.get(settings.BERLIN_FOOTER_URL, content=mock_footer)
    call_command("get_footer")
    footer = cache.get("footer")
