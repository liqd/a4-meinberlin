...
def test_searchprofile_filter_project_status(phase_factory, search_profile_factory):
    ...
    project_status_done = ProjectStatus.objects.get(status=2)
    project.save()

    search_profile = search_profile_factory()
    search_profile1 = search_profile_factory(status=project_status_done)
    search_profile2 = search_profile_factory()
    search_profile2.status.clear()
    assert search_profile1.status.first() == project_status_done
...
def test_searchprofile_filter_project_type(phase_factory, search_profile_factory):
    ...
    search_profile = search_profile_factory()
    search_profile_factory(project_types=ProjectType.objects.get(participation=2))
    search_profile2 = search_profile_factory()
...
@pytest.mark.django_db
def test_searchprofile_filter_query(phase_factory, search_profile_factory):
    phase, module, project, item = setup_phase(
        phase_factory, None, CollectFeedbackPhase
    )
    project.name = "A Project in Berlin"
    query, _ = KiezradarQuery.objects.get_or_create(text="Berlin")
    query1, _ = KiezradarQuery.objects.get_or_create(text="Error")
    search_profile = search_profile_factory()
    search_profile.query = query
    search_profile.save()
    search_profile1 = search_profile_factory()
...
@pytest.mark.django_db
def test_searchprofile_filter_querxy(
