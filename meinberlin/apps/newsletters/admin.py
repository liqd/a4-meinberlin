...
class NewsletterTaskAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        newsletter_content_type = ContentType.objects.get(
            app_label='meinberlin_newsletters', model='newsletter')
        qs = super().get_queryset(request).filter(
            creator_content_type=newsletter_content_type.pk)
        return qs

    def newsletter_id(self, task):
        url = reverse(
            'admin:meinberlin_newsletters_newsletter_change',
            args=[task.creator.id])
        return '<a href="{}">{}</a>'.format(url,
                                            task.creator.id)  # no escaping
