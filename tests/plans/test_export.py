...
def test_reply_to_mixin(
    plan_factory, project_factory, administrative_district, gis_point
):
    ...
    # ItemExportWithLocationMixin
    assert gis_point.x == export.get_location_lon_data(plan)
    assert gis_point.y == export.get_location_lat_data(plan)
    assert plan.point_label == export.get_location_label_data(plan)
